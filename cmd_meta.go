...
func helpJSON() map[string]any {
	...
	return map[string]any{
		...
		"commands": map[string]any{
			...
			"files ns create":    c([]string{"name"}, []string{"--backend <local|s3>", "--max-bytes <n>", "--allow-type <type>", "--public", "--verify-type", "--signing-key <key|auto>"}),
			...
			"files sign":         c([]string{"namespace", "name"}, []string{"--ttl <duration>", "--base-url <url>"}),
